...
//  FIX      : Removed limit_order (causes InvalidtoBuy errors on
//             CRASH500 multipliers). Exit managed via POC subscription.
...
// Minimum stake for CRASH500 multipliers is $1.00 on real accounts

const STAKE      = parseFloat(process.env.STAKE || '1.00'); // Default $1 (Deriv minimum for CRASH500)
const MULTIPLIER = parseInt(process.env.MULTIPLIER || '100');
const SYMBOL     = 'CRASH500';
const CANDLE_COUNT = 100;

// Fixed dollar risk (1:2 RR) — used for logging only now
...
function detectSignal(candles) {
  ...
  // ATR-based price levels (for logging reference only)
...
function placeTrade(signal) {
  ...
  log('TRADE', `Placing ${signal.dir} | Entry: ${signal.entry} | Stake: $${STAKE} | x${MULTIPLIER} | ATR: ${signal.atr}`);
  log('TRADE', `IB range: ${signal.ibLow} - ${signal.ibHigh} | Ref SL: ${signal.slPrice} | Ref TP: ${signal.tpPrice}`);

  // limit_order restored — dollar-based SL/TP works on x100,150,200,300,400
  // The earlier InvalidtoBuy error was from wrong multiplier (x40), NOT limit_order
  ws.send(JSON.stringify({
    ...
    parameters: {
      ...
      limit_order: {
        stop_loss:   SL_AMOUNT,   // = stake ($1 loss max)
        take_profit: TP_AMOUNT    // = stake x2 ($2 profit target)
      }
    }
  }));
}
...
function handleMsg(d) {
  // Log errors but don't crash — just skip this message
  if (d.error) {
    log('ERR', `${d.error.code}: ${d.error.message}`);
    // If a buy failed, make sure tradeOpen is reset so next signal can fire
    if (d.msg_type === 'buy') {
      tradeOpen      = false;
      tradeOpenSince = 0;
      log('SYS', 'Buy failed -- trade state reset, ready for next signal');
    }
    return;
  }
  ...
  switch (d.msg_type) {
    ...
    case 'authorize': {
      ...
      if (STAKE < 1.00) {
        log('WARN', 'Stake is below $1.00 -- Deriv CRASH500 minimum is $1.00. Trades will likely fail.');
      }
      log('CFG',  `Symbol: ${SYMBOL} | Stake: $${STAKE} | x${MULTIPLIER} | Ref SL: $${SL_AMOUNT} | Ref TP: $${TP_AMOUNT} | RR: 1:2`);
    ...
    case 'buy': {
      if (d.buy) {
        ...
        stats.trades++;
        log('OPEN', `Trade opened | ID: ${d.buy.contract_id} | Stake: $${STAKE} | x${MULTIPLIER}`);
        ws.send(JSON.stringify({
...
log('SYS', `Stake: $${STAKE} | x${MULTIPLIER}`);
