...
//             Stake clamped to MAX $0.90 (Deriv's CRASH500 cap).
// ================================================================
...
const MAX_STAKE  = 0.90; // Deriv hard cap for CRASH500 multipliers
const RAW_STAKE  = parseFloat(process.env.STAKE || '0.35');
const STAKE      = Math.min(RAW_STAKE, MAX_STAKE);
...
function placeTrade(signal) {
  ...
  // NOTE: limit_order is intentionally omitted.
  // Deriv rejects dollar-based SL/TP on CRASH500 multipliers via API.
  // Exit is managed automatically via proposal_open_contract subscription below.
  ...
  switch (d.msg_type) {
    ...
    case 'authorize': {
      ...
      if (RAW_STAKE > MAX_STAKE) {
        log('WARN', `STAKE env $${RAW_STAKE} exceeds CRASH500 cap $${MAX_STAKE} -- clamped to $${STAKE}`);
      }
